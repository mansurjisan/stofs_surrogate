...
# Training dates (29 days: Nov 1-29)
TRAINING_DATES = [
    # November 1-14 (new data)
...
TEMPORAL_FEATURES = 6   # η(t-1), dη/dt, + 4 tidal harmonics (sin/cos for M2, S2)
...
# Tidal harmonic periods (hours) - for phase features
M2_PERIOD = 12.42  # Principal lunar semi-diurnal
S2_PERIOD = 12.00  # Principal solar semi-diurnal

# Global epoch for continuous time (hours since this reference)
# Using 2025-01-01 00:00:00 UTC as epoch
...
class TemporalMemoryGNN(nn.Module):
    ...
    The tidal harmonics provide a global clock to reduce phase drift over long rollouts.
    """
    ...
    def __init__(
        ...
        # Input: η(t) + η(t-1) + dη/dt + tidal_harmonics(4) + static + forcing
        # = 1 + 1 + 1 + 4 + 4 + 3 = 14
    ...
    def forward(self, x, x_prev, dxdt, tidal_harmonics, static_features, forcing, edge_index, edge_attr):
        """
        Forward pass with temporal memory and tidal harmonics.

        Args:
            x: Current elevation η(t), shape [N, 1]
            x_prev: Previous elevation η(t-1), shape [N, 1]
            dxdt: Rate of change dη/dt, shape [N, 1]
            tidal_harmonics: Tidal phase features [N, 4] (sin/cos for M2, S2)
        ...
        """
        # Concatenate all node features including temporal memory and tidal harmonics
...
class TemporalMemoryDataset(Dataset):
    ...
    - tidal_harmonics: sin/cos of M2 and S2 tidal phases
    ...
    def __init__(self, mesh_data: Dict, date_data_list: List[Dict],
        ...
        # Precompute global hours from epoch for each date
        ...
        logger.info(f"  Temporal memory: using η(t-1), dη/dt, and tidal harmonics (M2, S2)")
    ...
    def __getitem__(self, idx):
        ...
        # Tidal harmonics - continuous global time (no reset per day)
        ...
        # Tidal harmonics for future timesteps (multi-step rollout)
        ...
        pres = forcing['pressure'][t].astype(np.float32)  # already normalized in preprocessing
...
def train_epoch(model, loader, optimizer, criterion, device, num_steps, grad_clip, scaler=None, amp_ctx=None):
    ...
    for batch in loader:
        ...
        for i in range(batch_size):
            ...
            with amp_ctx():
                # Step 1: Predict η(t+1) from η(t), η(t-1), dη/dt, tidal_harmonics
            ...
            # Backward pass (handles both AMP and non-AMP)
...
def evaluate_rollout(model, dataset, device, num_steps=48, amp_ctx=None):
    ...
    # Log which date we're evaluating
    ...
    for lead_hours in [1, 6, 12, 24, 48]:
        step_index = int(lead_hours / DT_HOURS) - 1  # e.g. 6h / 1.0h = 6 → index 5
