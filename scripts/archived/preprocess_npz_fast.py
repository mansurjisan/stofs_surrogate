#!/usr/bin/env python3
"""
Preprocess STOFS CWL data with GFS forcing from NPZ files. (FAST VERSION)

This version uses multiprocessing to parallelize processing across multiple dates.
...
Usage:
    python scripts/preprocess_npz_fast.py --cwl-dir /path/to/stofs_data --gfs-dir /path/to/gfs_forcing --output-dir /path/to/output
"""
...
def preprocess_date(date_str: str, mesh_data: dict, cwl_dir: Path, gfs_dir: Path):
    ...
    # Contiguous read (fast), then subset in memory
    ...
    return date_str, True, processed_data
...
def main():
    parser = argparse.ArgumentParser(description='Preprocess STOFS with GFS (NPZ format) - FAST')
    ...
    logger.info("=" * 70)
    logger.info("PREPROCESSING STOFS WITH GFS (NPZ FORMAT) - FAST VERSION")
    logger.info("=" * 70)
    ...
    first_cwl_date = dates_to_process[0]
    first_cwl_file = cwl_dir / first_cwl_date / 'stofs_2d_glo.t00z.fields.cwl.nc'
    if not first_cwl_file.exists():
        logger.error(f"Cannot build mesh. File not found: {first_cwl_file}")
        return

    mesh_data = select_wet_nodes(str(first_cwl_file), BBOX, args.max_nodes, MIN_DEPTH)
    ...
    logger.info(f"  Failed:     {fail_count}/{len(dates_to_process)}")
