#!/usr/bin/env python3
"""
25K Node STOFS-GNN Training Script for URSA H100

Optimized for:
- NVIDIA H100 (94GB VRAM)
- 25,000 nodes (Mid-Atlantic domain)
- Temporal memory architecture (fixes phase lag)

Expected training time: ~1-1.5 hours on H100

Usage:
    python scripts/train_25k_ursa_h100.py
"""
...
# ============================================================
# CONFIGURATION - URSA PATHS
# ============================================================

# Paths - Override with environment variables
DATA_DIR = Path(os.environ.get('STOFS_DATA_DIR', '/scratch5/purged/Mansur.Jisan/stofs_surrogate/data/processed_25k_midatl'))
OUTPUT_DIR = Path(os.environ.get('STOFS_OUTPUT_DIR', '/scratch5/purged/Mansur.Jisan/stofs_surrogate'))

# Training dates - dynamically loaded from data directory
# Set to None to auto-discover all dates
TRAINING_DATES = None  # Will be populated in main()

VAL_SPLIT = 0.15  # 15% of dates for validation

# ============================================================
# MODEL & TRAINING PARAMETERS (H100 Optimized)
# ============================================================
...
NUM_LAYERS = 6
STATE_DIM = 1
TEMPORAL_FEATURES = 6   # η(t-1), dη/dt, + 4 tidal harmonics
STATIC_NODE_FEATURES = 4
FORCING_FEATURES = 3

# Training - H100 optimized (larger batch, more workers)
EPOCHS = 100
BATCH_SIZE = 16         # Increased from 4 (H100 has 94GB vs A10G 24GB)
LEARNING_RATE = 2e-4
WEIGHT_DECAY = 1e-5
GRAD_CLIP = 1.0
NUM_WORKERS = 8         # More CPU cores on URSA
USE_AMP = True
...
# Global epoch for continuous time
...
class TemporalMemoryDataset(Dataset):
    """Dataset with temporal context for each sample."""
...
def evaluate_rollout(model, dataset, device, num_steps=48, amp_ctx=None):
    ...
    with torch.no_grad():
        for t in range(1, min(num_steps + 1, len(elev) - 1)):
            ...
            with amp_ctx():
                pred = model(current_cwl, current_prev, dxdt, tidal_harmonics_tensor,
...
def main():
    logger.info("=" * 70)
    logger.info("25K NODE TEMPORAL MEMORY GNN - URSA H100")
    logger.info("=" * 70)

    # Auto-discover dates from data directory
    all_dates = sorted([f.stem.replace('processed_', '')
                        for f in DATA_DIR.glob('processed_*.npz')])

    if not all_dates:
        logger.error(f"No processed files found in {DATA_DIR}")
        return
    ...
    logger.info(f"  Train dates: {len(train_dates)}, Val dates: {len(val_dates)}")
    logger.info(f"  BATCH_SIZE: {BATCH_SIZE}")
    logger.info(f"  HIDDEN_DIM: {HIDDEN_DIM}")
    logger.info(f"  NUM_LAYERS: {NUM_LAYERS}")
    logger.info(f"  EPOCHS: {EPOCHS}")

    checkpoint_dir = OUTPUT_DIR / 'outputs' / 'checkpoints_25k_h100'
    figure_dir = OUTPUT_DIR / 'outputs' / 'figures_25k'
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    figure_dir.mkdir(parents=True, exist_ok=True)

    # Load mesh (try both naming conventions)
    mesh_path = DATA_DIR / 'mesh.npz'
    if not mesh_path.exists():
        mesh_path = DATA_DIR / 'mesh_25k.npz'
    if not mesh_path.exists():
        logger.error(f"Mesh file not found in {DATA_DIR}")
        return
    ...
    train_dataset = TemporalMemoryDataset(mesh_data, train_data)
    val_dataset = TemporalMemoryDataset(mesh_data, val_data)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_loss, train_comp = train_epoch(model, train_loader, optimizer, criterion, device,
                                             num_steps=num_steps, grad_clip=GRAD_CLIP,
                                             scaler=scaler, amp_ctx=amp_ctx)
        val_loss, _ = validate(model, val_loader, criterion, device, amp_ctx=amp_ctx)
        ...
        if epoch % 5 == 0 or epoch == 1:
            logger.info(f"Epoch {epoch:3d}/{EPOCHS} | steps={num_steps} | "
    ...
    # Evaluate rollout
    logger.info("\nEvaluating rollout...")
    model.load_state_dict(torch.load(checkpoint_dir / 'best_model.pt', weights_only=False)['model_state_dict'])
    rollout_results, _, _ = evaluate_rollout(model, val_dataset, device, num_steps=48, amp_ctx=amp_ctx)
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.set_title('25K Node GNN Training (URSA H100)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.savefig(figure_dir / 'training_25k_h100.png', dpi=150)
    plt.close()
