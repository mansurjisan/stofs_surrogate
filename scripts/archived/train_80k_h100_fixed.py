#!/usr/bin/env python3
"""
Training Script for STOFS Surrogate - 80k nodes OPTIMIZED FOR H100
FIXED VERSION - Handles multi-step rollout memory requirements

Key Changes from original:
1. Dynamic batch size: BATCH_SIZE=4 for 1-step, BATCH_SIZE=2 for 2/3-step rollout
2. Gradient accumulation increases to compensate (8 -> 16 for multi-step)
3. Memory clearing between rollout steps
4. Resume from epoch 15 checkpoint

Usage:
    python scripts/train_80k_h100_fixed.py
"""
...
# ============================================================
# CONFIGURATION - FIXED FOR MULTI-STEP ROLLOUT
# ============================================================
...
EPOCHS = 100
# FIXED: Dynamic batch sizes for different rollout phases
BATCH_SIZE_1STEP = 4   # Batch size for 1-step rollout (epochs 1-15)
BATCH_SIZE_2STEP = 2   # Batch size for 2-step rollout (epochs 16-30)
BATCH_SIZE_3STEP = 1   # Batch size for 3-step rollout (epochs 31+) - OOM with batch=2
GRAD_ACCUM_1STEP = 8   # Gradient accumulation for 1-step (effective batch 32)
GRAD_ACCUM_2STEP = 16  # Gradient accumulation for 2-step (effective batch 32)
GRAD_ACCUM_3STEP = 32  # Gradient accumulation for 3-step (effective batch 32)
...
class BatchedSWEGraphBlock(nn.Module):
    ...
    def forward(self, h, edge_index, edge_attr):
        ...
        # Need to aggregate edge_msg[b, e, :] to aggr[b, row[e], :]
...
# ============================================================
# In-Memory Dataset (returns batched tensors)
# ============================================================
...
# FIXED: Added memory management for multi-step rollout
# ============================================================
...
def train_epoch_batched(model, loader, optimizer, criterion, device, num_steps,
    ...
    """
    TRUE BATCHED training with gradient accumulation.
    FIXED: Better memory management for multi-step rollout.
    """
    model.train()
    total_loss = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with amp_ctx:
            ...
            if num_steps >= 2:
                ...
                # FIXED: Clear intermediate tensors before second forward pass
                del wl, wl_mean, wl_std
                ...
                del dxdt_new, static_new, wl_norm
...
# ============================================================
# Main - FIXED with dynamic batch size
# ============================================================
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("H100-OPTIMIZED TRAINING - 80k nodes - FIXED FOR MULTI-STEP ROLLOUT")
    logger.info("=" * 70)
    logger.info(f"System RAM: {ram_gb:.1f} GB")
    logger.info(f"BATCH_SIZE: 1-step={BATCH_SIZE_1STEP}, 2-step={BATCH_SIZE_2STEP}, 3-step={BATCH_SIZE_3STEP}")
    logger.info(f"GRAD_ACCUM: 1-step={GRAD_ACCUM_1STEP}, 2-step={GRAD_ACCUM_2STEP}, 3-step={GRAD_ACCUM_3STEP}")
    logger.info(f"USE_COMPILE: {USE_COMPILE}")
    logger.info(f"NUM_WORKERS: {NUM_WORKERS}")

    checkpoint_dir = OUTPUT_DIR / 'outputs' / 'checkpoints_80k_h100'
    ...
    logger.info("\n" + "=" * 70)
    logger.info("STARTING FIXED MULTI-STEP ROLLOUT TRAINING")
    logger.info("=" * 70)

    # Track current loader batch size to recreate when needed
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        # Curriculum: rollout steps with separate batch sizes for each phase
        if epoch <= CURRICULUM_WARMUP_EPOCHS:
            num_steps = 1
            batch_size = BATCH_SIZE_1STEP
            grad_accum_steps = GRAD_ACCUM_1STEP
        elif epoch <= 2 * CURRICULUM_WARMUP_EPOCHS:
            num_steps = 2
            batch_size = BATCH_SIZE_2STEP
            grad_accum_steps = GRAD_ACCUM_2STEP
        else:
            num_steps = 3
            batch_size = BATCH_SIZE_3STEP
            grad_accum_steps = GRAD_ACCUM_3STEP

        # FIXED: Recreate data loaders if batch size changed
        if batch_size != current_batch_size:
            logger.info(f"  Adjusting batch_size: {current_batch_size} -> {batch_size}, "
                       f"grad_accum: {grad_accum_steps}")

            # Clear old loaders and force memory cleanup
            ...
            # Extra aggressive cleanup for 3-step transition
            if num_steps == 3:
                logger.info("  3-step rollout: aggressive memory cleanup...")
                gc.collect()
                torch.cuda.empty_cache()
