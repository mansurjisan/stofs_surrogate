#!/usr/bin/env python3
"""
A10G OPTIMIZED Multi-Date CWL GNN Training Script

This script is optimized for NVIDIA A10G (24GB VRAM) on AWS/ParallelWorks.
It uses your existing preprocessed data (15,000 nodes).

Key optimizations for A10G:
1. BATCH_SIZE = 8 with gradient accumulation
2. Mixed precision (FP16) training
3. Multi-worker data loading
4. cuDNN optimizations
5. No gradient checkpointing needed (enough VRAM)

Expected training time on A10G:
- 3 days: ~30-45 minutes (vs 8+ hours on RTX 3050)
- 15 days: ~3-5 hours (vs 30+ hours on RTX 3050)

Usage:
    # Set paths via environment variables or edit the script
    export STOFS_DATA_DIR=/path/to/processed_optimized
    export STOFS_OUTPUT_DIR=/path/to/output
    
    python train_cwl_gnn_a10g.py
"""
...
# ============================================================
# A10G OPTIMIZATIONS - Enable at startup
# ============================================================
...
# Paths - Your ParallelWorks paths
DATA_DIR = '/home/Mansur.Jisan/stofs_surrogate/data/processed_optimized'
OUTPUT_DIR = '/home/Mansur.Jisan/stofs_surrogate'

# Training dates
TRAINING_DATES = ['20251128', '20251129', '20251130']  # 3 days
# For 15 days, uncomment:
# TRAINING_DATES = [f'202511{d:02d}' for d in range(16, 31)]  # Nov 16-30

VAL_DATES = 1  # Last N dates for validation

# ============================================================
# A10G OPTIMIZED PARAMETERS
# ============================================================
...
# Training - A10G optimized
EPOCHS = 150
BATCH_SIZE = 8                    # ← Increased from 1
LEARNING_RATE = 3e-4              # ← Can be higher with larger batch
WEIGHT_DECAY = 1e-5
GRAD_CLIP = 1.0
...
class PhysicsLoss(nn.Module):
    """Physics-informed loss with mass conservation and smoothness."""
    ...
    def forward(self, pred, target, edge_index):
        ...
        # Mass conservation (clamped for stability)
        pred_sum = pred.sum()
...
class MultiDateCWLDataset(Dataset):
    """Dataset for multi-date CWL training."""
...
# ============================================================
# Training Functions - A10G Optimized with Batching
# ============================================================

def train_epoch(model, loader, optimizer, criterion, device, num_steps, grad_clip, scaler=None):
    """
    Train one epoch with batched processing and gradient accumulation.
    
    For graph data with shared topology, we process each sample individually
    but accumulate gradients across the batch before stepping.
    """
    model.train()
    total_loss = 0
    ...
    for batch in loader:
        ...
        # Edge data is shared across all samples
...
def main():
    logger.info("=" * 70)
    logger.info("A10G OPTIMIZED TRAINING - STOFS GNN SURROGATE")
    logger.info("=" * 70)
    ...
    if not mesh_path.exists():
        ...
        logger.error("Make sure DATA_DIR points to your preprocessed data directory")
        return
    ...
    # Create dataloaders with A10G optimizations
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Curriculum: 1 step for warmup, then 2 steps
        ...
        history['smooth'].append(train_comp['smooth'])
        ...
        if val_loss < best_val_loss:
            ...
            }, checkpoint_dir / 'best_a10g_model.pt')
    ...
    logger.info(f"Final MSE: {history['mse'][-1]:.6f}")
    logger.info(f"Final Mass loss: {history['mass'][-1]:.6f}")
    
    # Evaluate rollout
    logger.info("\nEvaluating rollout...")
    model.load_state_dict(torch.load(checkpoint_dir / 'best_a10g_model.pt')['model_state_dict'])
    ...
    axes[0, 1].semilogy(history['smooth'], label='Smooth', alpha=0.8)
    ...
    plt.tight_layout()
    plt.savefig(figure_dir / 'a10g_training_summary.png', dpi=150)
    plt.close()
    
    logger.info(f"\nFigures saved to: {figure_dir}")
    logger.info(f"Model saved to: {checkpoint_dir / 'best_a10g_model.pt'}")
    logger.info("\nDone!")
