...
Mid-Atlantic Domain: 25k nodes with GFS forcing
...
Usage:
    ...
    python scripts/preprocess_25k_gfs_v2.py --dates 20230108 20230109
    python scripts/preprocess_25k_gfs_v2.py --output-dir /path/to/output
"""
...
def preprocess_date_v2(date_str: str, mesh_data: dict, cwl_dir: Path, gfs_dir: Path):
    ...
    del gfs_hourly, dP_dx_grid, dP_dy_grid
    gc.collect()
...
_worker_mesh_file = None
_worker_cwl_dir = None
_worker_gfs_dir = None
_worker_tmp_dir = None
_worker_output_dir = None


def _init_worker(mesh_file, cwl_dir, gfs_dir, tmp_dir, output_dir):
    """Initialize worker process with shared data."""
    global _worker_mesh_file, _worker_cwl_dir, _worker_gfs_dir, _worker_tmp_dir, _worker_output_dir
    _worker_mesh_file = mesh_file
    _worker_cwl_dir = cwl_dir
    _worker_gfs_dir = gfs_dir
    _worker_tmp_dir = tmp_dir
    _worker_output_dir = output_dir


def _process_date_worker(date_str):
    """Worker function to process a single date."""
    global _worker_mesh_file, _worker_cwl_dir, _worker_gfs_dir, _worker_tmp_dir, _worker_output_dir

    try:
        # Load mesh data
        mesh_npz = np.load(_worker_mesh_file)
        mesh_data = {
            'global_indices': mesh_npz['global_indices'],
            'lon': mesh_npz['lon'],
            'lat': mesh_npz['lat'],
            'depth': mesh_npz['depth'],
        }

        cwl_dir = Path(_worker_cwl_dir)
        gfs_dir = Path(_worker_gfs_dir)

        data = preprocess_date_v2(date_str, mesh_data, cwl_dir, gfs_dir)
        if data is None:
            return date_str, False, "No data returned"

        # Determine output path
        if _worker_tmp_dir:
            out_file = Path(_worker_tmp_dir) / f'processed_{date_str}.npz'
        else:
            out_file = Path(_worker_output_dir) / f'processed_{date_str}.npz'
        ...
        # Copy from tmp to final destination if using tmp
        if _worker_tmp_dir:
            final_file = Path(_worker_output_dir) / f'processed_{date_str}.npz'
            shutil.copy2(str(out_file), str(final_file))
            out_file.unlink()
        ...
        import traceback
        return date_str, False, f"{e}\n{traceback.format_exc()}"


def main():
    parser = argparse.ArgumentParser(description='Preprocess STOFS 25k V2 - Enhanced Physics Features with GFS')
    ...
    parser.add_argument('--output-dir', type=str, default='/mnt/f/STOFS_TRAINING_DATA/processed_25k_gfs_v2',
    ...
    parser.add_argument('--use-tmp', action='store_true', default=False,
                        help='Write to /tmp first then copy to output (faster for network drives)')
    ...
    logger.info("=" * 70)
    logger.info("PREPROCESSING STOFS 25K V2 - ENHANCED PHYSICS FEATURES (GFS)")
    logger.info("=" * 70)
    logger.info("")
    ...
    logger.info("")
    logger.info(f"Domain: Mid-Atlantic {BBOX['lat_min']}-{BBOX['lat_max']}°N, {abs(BBOX['lon_max'])}-{abs(BBOX['lon_min'])}°W")
    ...
    dates_to_process = [d for d in dates if d not in existing]
    skipped = len(existing)
    logger.info(f"Dates to process: {len(dates_to_process)} (skipping {skipped} existing)")

    if not dates_to_process:
        logger.info("No dates to process!")
        return

    # Setup tmp directory if needed
    tmp_dir = None
    if args.use_tmp:
        tmp_dir = Path(tempfile.mkdtemp(prefix='stofs_preprocess_'))
        logger.info(f"Using tmp directory: {tmp_dir}")
    ...
    if args.workers > 1:
        ...
        with Pool(args.workers, initializer=_init_worker, initargs=init_args) as pool:
            results = pool.imap_unordered(_process_date_worker, dates_to_process)
            for i, (date_str, ok, error) in enumerate(results):
                if ok:
                    success += 1
                    logger.info(f"[{success + failed}/{len(dates_to_process)}] {date_str}: OK")
                else:
                    failed += 1
                    logger.error(f"[{success + failed}/{len(dates_to_process)}] {date_str}: FAILED - {error}")
    else:
        # Sequential processing
        for i, date_str in enumerate(dates_to_process):
            logger.info(f"[{i+1}/{len(dates_to_process)}]")
            try:
                data = preprocess_date_v2(date_str, mesh_data, cwl_dir, gfs_dir)
                if data is None:
                    failed += 1
                ...
                # Determine output path
                if tmp_dir:
                    out_file = tmp_dir / f'processed_{date_str}.npz'
                else:
                ...
                # Copy from tmp to final destination if using tmp
                if tmp_dir:
                    final_file = output_dir / f'processed_{date_str}.npz'
                    shutil.copy2(str(out_file), str(final_file))
                    out_file.unlink()

                success += 1
                logger.info(f"  Saved: processed_{date_str}.npz")
    ...
    if tmp_dir and tmp_dir.exists():
        ...
        logger.info(f"Cleaned up tmp directory")

    logger.info("=" * 70)
    logger.info(f"COMPLETED: {success} processed, {skipped} skipped, {failed} failed")
    ...
    logger.info("")
    logger.info("NEXT STEP: Use train_25k_v2.py with 6 tidal constituents")
    logger.info("=" * 70)
