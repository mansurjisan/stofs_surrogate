#!/usr/bin/env python3
"""
Training Script for Mid-Atlantic + New England STOFS Surrogate (40k nodes)

Pilot Training Plan:
- 30 dates from 2023 winter season (Jan-Feb)
- 40,000 nodes covering Norfolk VA to Portland ME
- Temporal memory model with tidal harmonics
- Validates pipeline before full 253-date training

Based on: train_25k_temporal_memory_v3.py

Usage:
    python scripts/train_midatlantic_40k_pilot.py
"""
...
# Paths - Mid-Atlantic + New England 40k dataset
# For ParallelWorks: /home/Mansur.Jisan/stofs_surrogate/data/processed_40k
# For local testing: /mnt/f/STOFS_TRAINING_DATA/processed
DATA_DIR = Path(os.environ.get('STOFS_DATA_DIR', '/home/Mansur.Jisan/stofs_surrogate/data/processed_40k'))
OUTPUT_DIR = Path(os.environ.get('STOFS_OUTPUT_DIR', '/home/Mansur.Jisan/stofs_surrogate'))

# Pilot training dates (30 days from 2023 - Jan 8 to Feb 6)
# Using available dates from preprocessing
TRAINING_DATES = [
    # January 2023
    '20230108', '20230109', '20230110', '20230111', '20230112',
    '20230113', '20230114', '20230115', '20230116', '20230117',
    '20230118', '20230119', '20230120', '20230121', '20230122',
    '20230123', '20230124', '20230125', '20230126', '20230127',
    '20230128', '20230129', '20230130', '20230131',
    # February 2023
    '20230201', '20230202', '20230203', '20230204', '20230205',
    '20230206',
]

VAL_DATES = 3  # Last 3 days for validation
...
def main():
    logger.info("=" * 70)
    logger.info("MID-ATLANTIC + NEW ENGLAND STOFS SURROGATE - PILOT TRAINING")
    logger.info("=" * 70)

    logger.info(f"\nDomain: 37-45N, 77-66W (Norfolk VA to Portland ME)")
    logger.info(f"Resolution: 40,000 nodes (~4.6 km grid spacing)")
    ...
    checkpoint_dir = OUTPUT_DIR / 'outputs' / 'checkpoints_40k_pilot'
    figure_dir = OUTPUT_DIR / 'outputs' / 'figures_40k_pilot'
    ...
    for date_str in train_dates:
        ...
        if data_path.exists():
            ...
            logger.info(f"  Loaded {date_str}: {data['elevation'].shape[0]} timesteps, {data['elevation'].shape[1]:,} nodes")
        else:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'config': {
                    ...
                    'domain': 'MidAtlantic_NewEngland_40k',
                }
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PILOT TRAINING COMPLETE")
    logger.info("=" * 70)
    ...
    axes[0, 0].set_title('Training Progress (40k Pilot)')
    axes[0, 0].legend()
    ...
    plt.tight_layout()
    plt.savefig(figure_dir / 'pilot_training_summary.png', dpi=150)
    plt.close()
