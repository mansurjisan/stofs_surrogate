...
NUM_EPOCHS = 50            # Shorter since fine-tuning
...
ROLLOUT_SCHEDULE = {
    6:  (1, 15, 1),     # Epochs 1-8: 6-step, batch=1
    12: (16, 30, 1),    # Epochs 9-15: 12-step, batch=1
    24: (31, 50, 1),   # Epochs 16-20: 24-step, batch=1
}
...
def main():
    ...
    # ========================================
    # Check for existing longrange checkpoints to resume from
    # ========================================
    ...
    # Determine which checkpoint is newer
    ckpt_epoch = 0
    best_epoch = 0

    if existing_ckpts:
        latest_ckpt = existing_ckpts[-1]
        ckpt_epoch = int(latest_ckpt.stem.split('_')[-1])

    if best_model_path.exists():
        best_ckpt = torch.load(best_model_path, map_location='cpu', weights_only=False)
        best_epoch = best_ckpt.get('epoch', 0)

    logger.info(f"Found checkpoints: numbered_epoch={ckpt_epoch}, best_model_epoch={best_epoch}")

    # PRIORITY: Always prefer numbered checkpoint (has full optimizer/scheduler state)
    # Only use best_model as fallback when no numbered checkpoint exists
    if ckpt_epoch > 0:
        # Use numbered checkpoint (has full optimizer/scheduler state)
        latest_ckpt = existing_ckpts[-1]
        logger.info(f"Resuming from numbered checkpoint: {latest_ckpt}")
        if best_epoch > ckpt_epoch:
            logger.info(f"  Note: best_model is from epoch {best_epoch}, but using numbered checkpoint for full optimizer state")
        ...
        logger.info(f"Resuming from epoch {start_epoch} (best_val={best_val_loss:.5f})")

    elif best_epoch > 0:
        # Fallback: Use best_model only if no numbered checkpoint exists
        logger.info(f"Resuming from best_model (epoch {best_epoch}) - no numbered checkpoint found")
        ckpt = torch.load(best_model_path, map_location=device, weights_only=False)
        model.load_state_dict(ckpt['model_state_dict'])
        start_epoch = ckpt['epoch'] + 1
        best_val_loss = ckpt.get('val_loss', float('inf'))

        # Advance scheduler to correct position (optimizer starts fresh)
        for _ in range(start_epoch - 1):
            scheduler.step()
        logger.info(f"Resuming from epoch {start_epoch} (best_val={best_val_loss:.5f})")
        logger.info("  Note: Optimizer state reset, scheduler advanced to correct LR")

    else:
        logger.info("No existing longrange checkpoints found, starting fresh fine-tuning")
