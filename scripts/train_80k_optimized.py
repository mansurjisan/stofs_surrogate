#!/usr/bin/env python3
"""
OPTIMIZED Training Script for 80k nodes - FAST VERSION

Key optimizations:
1. Epoch subsampling (use subset of dates per epoch)
2. Larger effective batch size via gradient accumulation
3. Progress logging every N batches
4. Memory-efficient data loading

Expected speedup: 10-20x faster per epoch
"""
...
# OPTIMIZATION 1: Effective batch size via accumulation
BATCH_SIZE = 2                  # Actual batch (memory limited)
ACCUMULATION_STEPS = 4          # Effective batch = 2 × 4 = 8
EFFECTIVE_BATCH_SIZE = BATCH_SIZE * ACCUMULATION_STEPS

# OPTIMIZATION 2: Subsample dates per epoch
DATES_PER_EPOCH = 50            # Use 50 dates per epoch (~20% of 253) - memory tested
EPOCHS = 150                    # More epochs to compensate for fewer dates

# OPTIMIZATION 3: Progress logging
LOG_EVERY_N_BATCHES = 100       # Log progress frequently
...
# ============================================================
# OPTIMIZED Dataset - Pre-loads subset of dates
# ============================================================

class EpochSubsampledDataset(Dataset):
    """
    Dataset that only loads a subset of dates per epoch.
    Much faster than loading all 222 dates!
    """
    
    def __init__(self, mesh_data: Dict, all_date_paths: List[tuple],
                 dates_per_epoch: int, eta_scale: float = 2.0, dt_hours: float = 1.0):
        
        self.eta_scale = eta_scale
        self.dt_hours = dt_hours
        self.all_date_paths = all_date_paths
        self.dates_per_epoch = min(dates_per_epoch, len(all_date_paths))
        ...
        self._compute_global_times_all()
        ...
        logger.info(f"EpochSubsampledDataset: {len(self.all_date_paths)} total dates")
        logger.info(f"  Using {self.dates_per_epoch} dates per epoch")
        logger.info(f"  Nodes: {self.num_nodes:,}, Edges: {self.edge_index.shape[1]:,}")

    def _compute_global_times_all(self):
        self.all_global_hours = {}
        for date_str, _ in self.all_date_paths:
            date_dt = datetime.strptime(date_str, '%Y%m%d')
            self.all_global_hours[date_str] = (date_dt - EPOCH_DATETIME).total_seconds() / 3600.0
    ...
    def resample_dates(self):
        """Randomly sample dates for this epoch and load their data."""
        # Clear old data
        self.elevations = []
        self.forcings = []
        self.samples = []
        ...
        # Sample random dates
        self.current_dates = random.sample(self.all_date_paths, self.dates_per_epoch)

        # Load data
        for date_idx, (date_str, file_path) in enumerate(self.current_dates):
            try:
                data = np.load(file_path)

                # Verify required keys exist
                required_keys = ['elevation', 'u10', 'v10', 'pressure']
                for key in required_keys:
                    if key not in data:
                        raise KeyError(f"Missing key '{key}' in {file_path}")

                self.elevations.append(data['elevation'])
                self.forcings.append({
                    'u10': data['u10'],
                    'v10': data['v10'],
        ...
        logger.info(f"  Loaded {len(self.current_dates)} dates, {len(self.samples)} samples")
    ...
    def __getitem__(self, idx):
        ...
        global_hour_t = self.all_global_hours[date_str] + t * self.dt_hours
...
def train_epoch_optimized(model, loader, optimizer, criterion, device, num_steps, 
                          grad_clip, scaler, amp_ctx, accumulation_steps, log_every):
    """Training with gradient accumulation and progress logging."""
    model.train()
    total_loss = 0
...
def main():
    logger.info("=" * 70)
    logger.info("OPTIMIZED TRAINING - 80k nodes (Epoch Subsampling)")
    logger.info("=" * 70)
    
    logger.info(f"\nOptimizations:")
    logger.info(f"  - Dates per epoch: {DATES_PER_EPOCH} (not all {222})")
    logger.info(f"  - Gradient accumulation: {ACCUMULATION_STEPS}x")
    logger.info(f"  - Effective batch size: {EFFECTIVE_BATCH_SIZE}")
    logger.info(f"  - Progress logging every {LOG_EVERY_N_BATCHES} batches")
    
    checkpoint_dir = OUTPUT_DIR / 'outputs' / 'checkpoints_80k_optimized'
    figure_dir = OUTPUT_DIR / 'outputs' / 'figures_80k_optimized'
    ...
    try:
        train_dataset = EpochSubsampledDataset(
            mesh_data, train_dates,
            dates_per_epoch=DATES_PER_EPOCH,
    ...
    # For validation, use a smaller fixed subset
    logger.info("\nCreating validation dataset...")
    try:
        val_dataset = EpochSubsampledDataset(
            mesh_data, val_dates[:10],  # Use first 10 val dates
            dates_per_epoch=10,
            eta_scale=ETA_SCALE, dt_hours=DT_HOURS
        )
    except Exception as e:
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        # Resample dates each epoch
        train_dataset.resample_dates()
        ...
        train_loss, train_comp = train_epoch_optimized(
            model, train_loader, optimizer, criterion, device,
            num_steps=num_steps, grad_clip=GRAD_CLIP, scaler=scaler,
            amp_ctx=amp_ctx, accumulation_steps=ACCUMULATION_STEPS,
            log_every=LOG_EVERY_N_BATCHES
        )

        val_loss, val_comp = validate(model, val_loader, criterion, device, amp_ctx)
