...
BATCH_SIZE = 4                  # Increased from 2 (fits with 15k nodes)
...
def train_epoch(model, loader, optimizer, criterion, device, num_steps, grad_clip):
    ...
    for batch in loader:
        ...
        # Get batch size and edge info (shared across samples)
        batch_size = batch['x'].shape[0]
        edge_index = batch['edge_index'][0].to(device) if batch['edge_index'].dim() == 3 else batch['edge_index'].to(device)
        edge_attr = batch['edge_attr'][0].to(device) if batch['edge_attr'].dim() == 3 else batch['edge_attr'].to(device)

        batch_loss = 0
        batch_components = {'mse': 0, 'mass': 0, 'smooth': 0}
        ...
        for i in range(batch_size):
            ...
            if num_steps >= 2:
                ...
                depth = static[:, 2:3] * 1.0  # Approximate
            ...
            batch_loss += loss / batch_size  # Average over batch
            for k in batch_components:
                batch_components[k] += components[k] / batch_size
        ...
        for k in total_components:
            total_components[k] += batch_components[k]
        sample_count += 1

    return total_loss / sample_count, {k: v / sample_count for k, v in total_components.items()}
